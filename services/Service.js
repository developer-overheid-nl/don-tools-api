...
class Service {
  ...
  static getMockModule(serviceName) {
    ...
    if (!Object.hasOwn(Service.mockModules, serviceName)) {
      ...
      try {
        ...
        logMockEvent("warn", "mock.module.load_failed", "Mock module could not be loaded", serviceName, undefined, {
          error: {
  ...
  static async applyMock(serviceName, operationId, params) {
    ...
    if (!mockModule) {
      ...
      if (generated !== undefined) {
        logMockEvent(
          "info",
          "mock.response.autogenerated",
          "Using autogenerated mock response",
          serviceName,
          operationId,
        );
    ...
    try {
      ...
      if (Service.isErrorResponse(result)) {
        logMockEvent("info", "mock.response.custom_error", "Using custom mock error response", serviceName, operationId);
        return { action: "reject", value: result };
      }
      logMockEvent("info", "mock.response.custom", "Using custom mock response", serviceName, operationId);
      return { action: "resolve", value: Service.normalizeSuccessResponse(result) };
    } catch (error) {
      if (Service.isErrorResponse(error)) {
        logMockEvent("warn", "mock.handler.rejected", "Mock handler rejected the request", serviceName, operationId);
  ...
  static getApiDoc() {
    ...
    try {
      ...
      logger.warn("OpenAPI specification for autogenerated mocks could not be loaded", {
        event: "mock.openapi.load_failed",
        error: {
          message: error?.message,
          ...(error?.stack ? { stack: error.stack } : {}),
        },
